...
const dateInChargeMonth=(transactionDate:string,chargeDate:string)=>{if(!transactionDate||!chargeDate)return chargeDate;const[year,month]=chargeDate.split("-").map(Number),day=Number(transactionDate.slice(8,10)),lastDay=new Date(year,month,0).getDate();return `${year}-${String(month).padStart(2,"0")}-${String(Math.min(day,lastDay)).padStart(2,"0")}`};
...
export async function parseLeumiFile(file:File,categories:Category[]){
  ...
  for(const sheetName of workbook.SheetNames){
    ...
    for(const row of rows.slice(headerIndex+1)){
      const chargeDate=dateValue(row[dateCol]),transactionDate=transactionDateCol>=0?dateValue(row[transactionDateCol]):"",date=isMax?dateInChargeMonth(transactionDate,chargeDate):chargeDate,description=clean(row[descriptionCol]);if(!date||!description){if(row.some(Boolean)&&clean(row[0])!=="סך הכל")skipped++;continue}
