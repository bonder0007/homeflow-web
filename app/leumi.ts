...
export type ImportRow={date:string;description:string;amount:number;type:"expense"|"income";categoryId:number|null;source:"leumi"|"max";sourceCategory?:string;notes?:string;isAccountTransfer?:boolean};
...
const aliases={
  ...
  debit:["חובה","בחובה","סכוםחובה","משיכה","חיוב","debit"],
  credit:["זכות","בזכות","סכוםזכות","הפקדה","credit"],
...
const dateValue=(value:unknown,swapExcelDateParts=false)=>{let d:Date|undefined;if(value instanceof Date)d=swapExcelDateParts?new Date(value.getFullYear(),value.getDate()-1,value.getMonth()+1):value;else if(typeof value==="number"){const parsed=XLSX.SSF.parse_date_code(value);if(parsed)d=swapExcelDateParts?new Date(parsed.y,parsed.d-1,parsed.m):new Date(parsed.y,parsed.m-1,parsed.d)}else{const s=clean(value),m=s.match(/^(\d{1,2})[/.\-](\d{1,2})[/.\-](\d{2,4})$/);if(m)d=new Date(Number(m[3])<100?2000+Number(m[3]):Number(m[3]),Number(m[2])-1,Number(m[1]));else if(s)d=new Date(s)}if(!d||Number.isNaN(d.getTime()))return "";return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}-${String(d.getDate()).padStart(2,"0")}`};
...
const isTransferDescription=(description:string)=>/העברה דיגיטל|הע\. אינטרנט|העברת משכורת/i.test(description);
const leumiHasSwappedExcelDates=(rows:unknown[][],dateCol:number)=>rows.some(row=>row[dateCol] instanceof Date)&&rows.some(row=>{const match=clean(row[dateCol]).match(/^(\d{1,2})[/.\-](\d{1,2})[/.\-]\d{2,4}$/);return Boolean(match&&Number(match[1])>12)});

export async function parseLeumiFile(file:File,categories:Category[]){
  const workbook=XLSX.read(await file.arrayBuffer(),{type:"array",cellDates:true});
  const parsed:ImportRow[]=[];let skipped=0,recognizedSheets=0;const excludedRentTransferDates:string[]=[];
  for(const sheetName of workbook.SheetNames){
    ...
    const dateCol=isMax?findColumn(headers,["תאריךחיוב"]):findColumn(headers,aliases.date),transactionDateCol=isMax?findColumn(headers,["תאריךעסקה"]):-1,descriptionCol=findColumn(headers,aliases.description),debitCol=findColumn(headers,aliases.debit),creditCol=findColumn(headers,aliases.credit),amountCol=isMax?findColumn(headers,["סכוםחיוב"]):findColumn(headers,aliases.amount),sourceCategoryCol=findColumn(headers,["קטגוריה"]),notesCol=findColumn(headers,["הערות"]),swapLeumiDateParts=!isMax&&leumiHasSwappedExcelDates(rows.slice(headerIndex+1),dateCol);
    for(const row of rows.slice(headerIndex+1)){
      const chargeDate=dateValue(row[dateCol],swapLeumiDateParts),transactionDate=transactionDateCol>=0?dateValue(row[transactionDateCol]):"",date=isMax?dateInStatementMonth(transactionDate,chargeDate):chargeDate,description=clean(row[descriptionCol]);if(!date||!description){if(row.some(Boolean)&&clean(row[0])!=="סך הכל")skipped++;continue}
      ...
      if(!isMax&&type==="expense"&&amount===7500&&isTransferDescription(description)){excludedRentTransferDates.push(date);continue}
  ...
  return{rows:parsed,skipped,excludedRentTransfers:excludedRentTransferDates.length,excludedRentTransferDates,provider:parsed.every(r=>r.source==="max")?"MAX":parsed.every(r=>r.source==="leumi")?"לאומי":"משולב"};
}
